...
<body>
	...
	<div class="editform">
		...
		<form action="${ctxPath}/orderManage/OrderManageController"
			...
			<div class="info">
				<span class="text">訂單編號:</span> <input name="orderNO" value="${order.orderNO}"
					type="text" readonly="readonly" class="fixed" > <span class="text">會員編號:</span>
				<input name="memId" value="${order.memId}" type="text"
					readonly="readonly" class="fixed" > <span class="text">訂單日期:</span>
				<p class="orderTime">
					<fmt:formatDate value="${order.orderTime}" pattern="yyyy-MM-dd" />
				</p>
				<span class="text">訂單狀態:</span> <select class="status" name="orderStatus">
				...
				<input name="orderPrice" value="${order.actualPrice }" type="text" readonly="readonly"> 
				<span class="text">收件人:</span>
				 <c:if test="${not empty ErrorMap && ErrorMap['recipient'] != null}">
        			<span class="error">${ErrorMap['recipient']}</span>
   				 </c:if>
				<input name="recipient" value="<%=request.getParameter("recipient") %>" type="text">
				<span class="text">收件地址:</span> 
				 <c:if test="${not empty ErrorMap && ErrorMap['recipientAddress'] != null}">
        			<span class="error">${ErrorMap['recipientAddress']}</span>
   				 </c:if>
				<input name="recipientAddress" value="<%=request.getParameter("recipientAddress") %>" type="text"> 
				<span class="text">連絡電話:</span>
				 <c:if test="${not empty ErrorMap && ErrorMap['phone'] != null}">
        			<span class="error">${ErrorMap['phone']}</span>
   				 </c:if>
				<input name="phone" value="<%=request.getParameter("phone") %>" type="text">
				<input name="action" value="editcomplete" type="hidden">
